...
int GameTests::setRand()
{
    int num = rand()%50;
    return num;
}

time_t GameTests::Clock()
{
  time_t now;
  double seconds;
...
void GameTests::setJellyGrid()
{
    ...
    for (auto count = 1; count<(gridBlocks.size()-5);count++)
    {
        ...
        {
            auto randNum= rand()%10; //10% chancw of being a jellyfish
            if(randNum == 1 )
            {
...
void GameTests::newJelly(float xPos, float yPos)
{
...
void GameTests::newBlast(float x, float y)
{
...
//--------------------------------------------------------------------------------------------------------------------------------
const bool GameTests::running() const
{
    return window->isOpen();
}

void GameTests::checkEvents()
{
...
void GameTests::setScoreDisplay()
{
...
// Calculations
//--------------------------------------------------------------------------------------------------------------------------------
void GameTests::monsterVictoryCheck()  //if monster (Octopus or centipede train) hits player subtract life
{
...
void GameTests::centiChangeDirCheck()  //if monster hits bottom starts coming up and vice versa
{
    for (auto count = 0; count< centiVector.size();count++)
    {
...
void GameTests::blastDeleteCheck() //delete blast object when out of bounds
{
...
void GameTests::collisionsCheck() //When lightning blast collides when other objects
{
    for (auto count=0;count<BlastArsenal.size();count++)
    {
        ...
        for(auto count3=0;count3<BombVector.size();count3++)
        {
            ...
            {
                //BombVector.erase(begin(BombVector)+count3);
...
void GameTests::centiDestroyedCheck() //if player destroys centipede game ends
{
...
void GameTests::jellyCollisionCheck() //collisions with jellyfish
{
...
void GameTests::BombCollisions() //collisions between DDT bomb explosion,  Jellyfish
{
    for(auto count2=0;count2<centiVector.size();count2++)
    {
        for (auto count3 =0; count3 < BombVector.size();count3++)
    ...
    for (auto count =0; count < BombVector.size();count++)
    {
        if(Exploded[count]==true)
        {
            ...
            if(BombVector[count].getBombBounds().intersects(AnchorVector[0].getBounds()))
            {
                std::cout<<"Deleting anchor"<<std::endl;
                //destroy Anchor
...
void GameTests::anchorPosCheck() //checks if Anchor within player movement area to spawn new Jellyfish at random
{
    auto chanceSpawnJelly = rand()%20;
    if( AnchorVector[0].getPos().y > 450.f && chanceSpawnJelly == 1) //within player movement area
    {
...
// TEST CASE Accessors
//--------------------------------------------------------------------------------------------------------------------------------
Player GameTests::accessPlayer()
{
    return player;
}

std::vector<Centipede> GameTests::accessCenti()
{
    return centiVector;
}

std::vector<Octopus> GameTests::accessOct()
{
    return OctopusVector;
}

std::vector<Bomb> GameTests::accessBombs()
{
    return BombVector;
}

std::vector<bool> GameTests::accessExplosion()
{
    return Exploded;
}

std::vector<Blast> GameTests::accessBlast()
{
    return BlastArsenal;
}
std::vector<Jelly> GameTests::accessJelly()
{
    return JellyVector;
}
std::vector<Anchor> GameTests::accessAnchor()
{
    return AnchorVector;
}

int GameTests::accessLives()
{
    return lives;
}
std::vector<int> GameTests::accessHitCount()
{
    return hitCount;
}
std::vector<bool> GameTests::accessIsHead()
{
    return isHead;
}
//--------------------------------------------------------------------------------------------------------------------------------
// TEST CASE Modifiers
//--------------------------------------------------------------------------------------------------------------------------------
void GameTests::modPlayerPos(float x, float y)
{
    player.setPos(x,y);
}
void GameTests::modCentiPos(float x, float y)
{
    centiVector.at(0).setPos(x,y);
}
void GameTests::modOctPos(float x, float y)
{
    OctopusVector.at(0).setOctPos(x,y);
}
void GameTests::modBombPos(float x, float y,int pos)
{
    setBombs();
    if(BombVector.size()<4)
    {
        BombVector.at(pos).setPos(x,y);
    }
}
void GameTests::modExplodedPos()
{
    Exploded.push_back(true);
}
void GameTests::modBlastPos(float x, float y)
{
    BlastArsenal.at(0).setPos(x,y);
}
void GameTests::modJellyPos(float x, float y)
{
    JellyVector.at(0).setPos(x,y);
}
void GameTests::modAnchorPos(float x, float y)
{
    AnchorVector.at(0).setPos(x,y);
}

//--------------------------------------------------------------------------------------------------------------------------------
// Update & Refresh
//--------------------------------------------------------------------------------------------------------------------------------
void GameTests::refresh()
{
    //CHECKS
    ...
    //UPDATE GRID
    if(gridBlocks.empty()==false)
    {
        for (auto count = 0; count<gridBlocks.size();count++)
        {
            for (auto count2 = 0; count2<gridBlocks[count].size(); count2++)
            {
                gridBlocks[count][count2].manifest(window);
            }
        }
    }
